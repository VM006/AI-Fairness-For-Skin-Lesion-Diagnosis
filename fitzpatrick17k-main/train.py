...
class SkinDataset():
    ...
    def __getitem__(self, idx):
        ...
        img_name = r"{}".format(img_name) # Convert dir to raw string to hopefully not get anymore file not found errors
        img_name = img_name + r".jpg" # Add JPG extension name to hopefully really not get the file not found errors
        print(img_name) # ***********INSERTED BY VM FOR DEBUGGING PURPOSES***********
...
def custom_load(
        batch_size=256,
        num_workers=3, # Changed from 20 so that Windows doesn't complain
        train_dir='',
        val_dir='',
        image_dir=r"E:\finalfitz17k"):
...
if __name__ == '__main__':
    ...
    if dev_mode == "dev":
        df = pd.read_csv(r"C:\Users\Vishn\Documents\KingProject\Program\fitzpatrick17k-main\fitzpatrick17k.csv").sample(1000)
    else:
        df = pd.read_csv(r"C:\Users\Vishn\Documents\KingProject\Program\fitzpatrick17k-main\fitzpatrick17k.csv")
    ...
    for holdout_set in ["expert_select","random_holdout", "a12", "a34","a56", "dermaamin","br"]:
        ...
        for indexer, label in enumerate(["low"]):
            ...
            class_weights = torch.FloatTensor(weights).device
